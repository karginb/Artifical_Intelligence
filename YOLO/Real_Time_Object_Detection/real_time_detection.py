...
while True:
    ...
    for box in boxes:
        ...
        if score > confidence_score:
            ...
            cv2.rectangle(frame, (x1, y1 - 10 - label_size[1]), (x1 + label_size[0], int(y1 + baseline - 10)), box_color, cv2.FILLED)
    ...
    avg_fps = int(average_fps)

    cv2.rectangle(frame, (10, 2), (280, 50), background_color, -1)
    cv2.putText(frame, "FPS: " + str(fps), (20, 40), font, 1.5, text_color_b, thickness = 3)
...
save_path = "results/test_vid_res-3.avi"
writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*"XVID"), int(avg_fps), (width, height))
